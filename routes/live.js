...
router.get('/live', (req, res) => {
  ...
  const requestAsObservable = Rx.bindNodeCallback(request.get);
  const observables = urls.map((url) => requestAsObservable(url));

  Rx.forkJoin(...observables).subscribe((data) => {
    context.one = data[0];
    context.two = data[1];
    context.three = data[2];
    context.four = data[3];
    context.five = data[4];
    console.log(data);
  });

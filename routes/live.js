...
router.get('/live', (req, res) => {
  ...
  var requestAsync = function (url) {
    return new Promise((resolve, reject) => {
      if (url.startsWith('https://api0.solar')) {
        var req = request2(url, (err, response, body) => {
  ...
  var getParallel = async function () {
    ...
    try {
      var data = await Promise.all(urls.map(requestAsync)).then(function () {
        console.log(data);
        // context.one = JSON.parse(data[0]);
        // context.two = convert.xml2js(data[1], { compact: true });
        // context.three = convert.xml2js(data[2], { compact: true });
        // context.four = JSON.parse(data[3]);
        // context.five = convert.xml2js(data[4], { compact: true });
        // console.log(context);
      });
    } catch (err) {
      console.error(err);
  ...
  const ccgt =
    context.two.response.responseBody[0].responseList[0].item[0].currentMW[0] /
    1000;
  const coal =
    context.two.response.responseBody[0].responseList[0].item[3].currentMW[0] /
    1000;
  const nuclear =
    context.two.response.responseBody[0].responseList[0].item[4].currentMW[0] /
    1000;
  const wind =
    context.two.response.responseBody[0].responseList[0].item[5].currentMW[0] /
    1000;
  ...
  const pumpHydro =
    context.two.response.responseBody[0].responseList[0].item[6].currentMW[0] /
    1000;
  const hydro =
    context.two.response.responseBody[0].responseList[0].item[7].currentMW[0] /
    1000;
  const total =
  ...
  const time = moment(context.two.response.responseBody[0].dataLastUpdated[0])
    .tz('Europe/London')
    .format('h:mm A');
  ...
  const itsdo =
    context.three.response.responseBody[0].responseList[0].item[demLength - 1]
      .demand[0] / 1000;
  ...
  const embeddedWindToday = windToday.map((item, i) =>
    Number(item * onshorePercent * onShoreScaleFactor * 0.9 + item).toFixed(3)
  );
